...
//			}
			VideoStop(&videoCapt);
			EdgeDetection(pFrames[videoCapt.curFrame], pFrames[nextFrame], videoCapt.timing.HActiveVideo, videoCapt.timing.VActiveVideo, DEMO_STRIDE);
			VideoStart(&videoCapt);
			DisplayChangeFrame(&dispCtrl, nextFrame);
			Xil_DCacheEnable();
			break;
...
void EdgeDetection(u8 *srcFrame, u8 *destFrame, u32 width, u32 height, u32 stride)
{
    u32 x, y;
    u32 lineStart = 0;
    ...
    {
        for (x = 3; x < (width - 1) * 3; x += 3)
        {
            // Apply the Sobel operator for edge detection
            int gx = srcFrame[x + lineStart - stride - 3] - srcFrame[x + lineStart + stride + 3] +
                     2 * srcFrame[x + lineStart - 3] - 2 * srcFrame[x + lineStart + 3] +
                     srcFrame[x + lineStart - stride + 3] - srcFrame[x + lineStart + stride - 3];

            int gy = srcFrame[x + lineStart - stride - 3] + 2 * srcFrame[x + lineStart - stride] +
                     srcFrame[x + lineStart - stride + 3] - srcFrame[x + lineStart + stride - 3] -
                     2 * srcFrame[x + lineStart + stride] - srcFrame[x + lineStart + stride + 3];

            // Calculate gradient magnitude
            int magnitude = abs(gx) + abs(gy);

            // Clamp magnitude to 255 (8-bit)
            destFrame[x + lineStart] = (magnitude > 255) ? 255 : magnitude;
            destFrame[x + lineStart + 1] = (magnitude > 255) ? 255 : magnitude;
            destFrame[x + lineStart + 2] = (magnitude > 255) ? 255 : magnitude;
        }
        lineStart += stride;
    }
